...
- **Multi-binding let**: Elm-style `let` blocks with multiple bindings and one `in`. Subsequent bindings at the same column as the first name are part of the same block: `let a = 1\n    b = 2\nin\na + b`. Parser-only — desugars to nested `Let` AST nodes. Only for non-recursive `let` (use `and` for mutual recursion). `letBindCol` (exact-match offside) set to `nameCol` during body parsing to prevent the body from consuming the next binding's name as a function argument; unlike `caseArmCol` (`<=`), this allows arguments wrapping to columns below `nameCol`.
...
- **Multi-binding let**: Elm-style `let` blocks with multiple bindings and one `in`. Parser-only change — desugars to nested `Let` AST nodes (typechecker and eval untouched). Subsequent bindings at the same column as the first binding name are part of the same block. Uses `letBindCol` (exact-match offside in `parseApp`) to prevent body parsing from consuming the next binding's name as a function argument; unlike `caseArmCol` (`<=`), allows arguments wrapping below `nameCol`. Only for non-recursive `let`; `let rec` uses `and` for mutual recursion. Old chained `let...in...let...in` syntax unchanged. Function bindings (`let f x = ...`) work. Single-binding `let` is the degenerate case (1-element list).
