...
- **Implicit let continuation**: `let x = expr` without explicit `in` — when the `let` is indented inside a bounded context (case arm, function body) and the next token is at the same column as `let`, it is parsed as an implicit continuation (the `InExpr`). This enables `let _ = doSomething\nnextExpr` without `in`. Body parsing is bounded by `caseArmCol = letCol` so the RHS cannot eat tokens at `letCol` or before. Does NOT apply at top level or in test bodies (where `letCol == caseArmCol`), since those contexts loop over independent expressions.
