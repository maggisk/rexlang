...
field key decoder =
    Decoder \json ->
        match json
            when JObj pairs ->
                match getField key pairs
                    when Just val ->
                        match run decoder val
                            when Ok v ->
                                Ok v
                            when Err e ->
...
test "field decoder" =
    ...
    let missingResult =
        match decodeString (field "missing" string) json
            when Err e ->
...
test "index decoder" =
    ...
    let indexErr =
        match decodeString (index 5 int) json
            when Err e ->
...
test "list decoder" =
    ...
    let listErr =
        match decodeString (list int) """[1, "x"]"""
            when Err e ->
                e.path == ["[1]"] && e.message == "expected an Int"
            when _ ->
                false
...
test "nullable decoder" =
    ...
    let nullErr =
        match decodeString (nullable int) "\"hi\""
            when Err e ->
...
test "optionalField decoder" =
    ...
    let optErr =
        match decodeString (optionalField "age" int) json3
            when Err e ->
...
test "succeed and fail" =
    ...
    let failErr =
        match decodeString (fail "nope") "null"
            when Err e ->
                e.message == "nope"
            when _ ->
                false
